...
local function get_config()
    ...
    if not url or url == "" then
        return nil, nil, { title = "Binary Sensors", items = { error_item({
...
lark.register({
    on_run = function()
        ...
        if not resp or resp.status ~= 200 then
            return { title = "Binary Sensors", items = { ha_http_error(resp, url .. "/api/states") } }
        end
        local ok, states = pcall(lark.json.decode, resp.body)
        if not ok or not states then
            return { title = "Binary Sensors", items = { error_item({
