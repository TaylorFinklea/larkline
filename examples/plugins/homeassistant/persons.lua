-- Persons — person entities showing home/away status.
...
local function get_config()
    ...
    if not url or url == "" then
        return nil, nil, { title = "Persons", items = {
...
lark.register({
    on_run = function()
        ...
        if not resp or resp.status ~= 200 then
            ...
            return { title = "Persons", items = { { label = "HA API error: " .. tostring(code), icon = "!" } } }
        end
        local ok, states = pcall(lark.json.decode, resp.body)
        if not ok or not states then
            return { title = "Persons", items = { { label = "Invalid JSON from HA", icon = "!" } } }
        end
        ...
        for _, entity in ipairs(states) do
            ...
            if not eid:match("^person%.") then goto next_label end

            local name = friendly_name(entity)
            local state = tostring(entity.state or "unknown")
            local icon = state == "home" and "👤" or "🏃"
            local detail = state .. "  " .. eid
        ...
        if #items == 0 then
            return { title = "Persons", items = { { label = "No persons found", icon = "📭" } } }
        end
        return { title = "Persons — " .. #items, items = items }
    end,
})
