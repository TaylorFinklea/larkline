-- Covers — garage doors, blinds, shades, and other cover entities.
...
local function get_config()
    ...
    if not url or url == "" then
        return nil, nil, { title = "Covers", items = {
...
lark.register({
    on_run = function()
        ...
        if not resp or resp.status ~= 200 then
            ...
            return { title = "Covers", items = { { label = "HA API error: " .. tostring(code), icon = "!" } } }
        end
        local ok, states = pcall(lark.json.decode, resp.body)
        if not ok or not states then
            return { title = "Covers", items = { { label = "Invalid JSON from HA", icon = "!" } } }
        end
        ...
        for _, entity in ipairs(states) do
            ...
            if not eid:match("^cover%.") then goto next_label end
            ...
            items[#items + 1] = {
                ...
                copy_text = eid,
                actions = {
                    { label = "Open", kind = "shell",
                      args = curl_service(url, token, "cover/open_cover", body),
                       },
                    { label = "Close", kind = "shell",
                      args = curl_service(url, token, "cover/close_cover", body),
                       },
                    { label = "Stop", kind = "shell",
                      args = curl_service(url, token, "cover/stop_cover", body),
                       },
                    { label = "Toggle", kind = "shell",
                      args = curl_service(url, token, "cover/toggle", body),
                       },
        ...
        if #items == 0 then
            return { title = "Covers", items = { { label = "No covers found", icon = "📭" } } }
        end
        return { title = "Covers — " .. #items, items = items }
    end,
})
