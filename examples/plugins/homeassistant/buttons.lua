...
local function get_config()
    ...
    if not url or url == "" then
        return nil, nil, { title = "Buttons", items = {
...
lark.register({
    on_run = function()
        ...
        if not resp or resp.status ~= 200 then
            ...
            return { title = "Buttons", items = { { label = "HA API error: " .. tostring(code), icon = "!" } } }
        end
        local ok, states = pcall(lark.json.decode, resp.body)
        if not ok or not states then
            return { title = "Buttons", items = { { label = "Invalid JSON from HA", icon = "!" } } }
        end
