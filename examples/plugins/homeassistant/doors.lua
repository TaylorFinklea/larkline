-- Doors — binary_sensor entities with device_class "door".
...
local function get_config()
    ...
    if not url or url == "" then
        return nil, nil, { title = "Doors", items = {
...
lark.register({
    on_run = function()
        ...
        if not resp or resp.status ~= 200 then
            ...
            return { title = "Doors", items = { { label = "HA API error: " .. tostring(code), icon = "!" } } }
        end
        local ok, states = pcall(lark.json.decode, resp.body)
        if not ok or not states then
            return { title = "Doors", items = { { label = "Invalid JSON from HA", icon = "!" } } }
        end
        ...
        for _, entity in ipairs(states) do
            ...
            if attrs.device_class ~= "door" then goto next_label end
        ...
        if #items == 0 then
            return { title = "Doors", items = { { label = "No door sensors found", icon = "📭" } } }
        end
        return { title = "Doors — " .. #items, items = items }
    end,
})
