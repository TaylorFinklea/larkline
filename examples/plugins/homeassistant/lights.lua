...
local function get_config()
    ...
    if not url or url == "" then
        return nil, nil, { title = "Lights", items = { error_item({
...
lark.register({
    on_run = function()
        ...
        if not resp or resp.status ~= 200 then
            return { title = "Lights", items = { ha_http_error(resp, url .. "/api/states") } }
        end
        local ok, states = pcall(lark.json.decode, resp.body)
        if not ok or not states then
            return { title = "Lights", items = { error_item({
        ...
        for _, entity in ipairs(states) do
            ...
            if hidden_entities_set[eid] then goto next_light end
            ...
            if hidden_states_set[state] then goto next_light end
