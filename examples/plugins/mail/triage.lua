-- Mail: Triage -- inbox with mutating actions per row.
--
    ...
    on_run = function()
        ...
        if err then
            local help = err:find("not authorized", 1, true)
                ...
                or nil
            return { title = "Triage", items = { error_item("Mail error: " .. err, help) } }
        end

        if not messages or #messages == 0 then
            return {
                title = "Triage",
                items = { { icon = "🎉", label = "Inbox zero — nothing to triage", actions = {} } },
            }
        end
        ...
        return { title = "Triage", items = items }
    end,
})
