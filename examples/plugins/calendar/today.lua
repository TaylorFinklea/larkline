-- Calendar: Today -- structured events for today.
--
-- Same backend as schedule.lua but scoped to today's date range. Single
...
local function icalbuddy_fallback(range)
    ...
    local raw = lark.exec("icalbuddy", {
        ...
        "-npn", "-nrd", range or "eventsToday",
    })
    if not raw or raw:match("^%s*$") then
        return { { icon = "🎉", label = "Nothing on the calendar today", actions = {} } }
    end
...
lark.register({
    on_run = function()
        if not has_helper() then
            return { title = "Today", items = icalbuddy_fallback("eventsToday") }
        end

        local start_iso = iso_for_offset(0)
        local end_iso = iso_for_offset(1)
        if not start_iso or not end_iso then
            return { title = "Today", items = { error_item("date(1) shell command failed") } }
        end
        ...
        if err then
            local help = err:find("calendar access denied", 1, true)
                ...
                or nil
            return { title = "Today", items = { error_item(err, help) } }
        end
        ...
        if #events == 0 then
            return {
                title = "Today",
                items = { { icon = "🎉", label = "Nothing on the calendar today", actions = {} } },
            }
        end
        ...
        return { title = "Today", items = items }
    end,
})
