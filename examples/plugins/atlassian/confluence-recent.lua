-- Atlassian: Recent Pages — recently updated Confluence pages.
...
local function http_error_output(title, status, body)
    local msg = (status == 401) and "401 Unauthorized — token revoked or expired"
        ...
        or (status == 404) and "404 Not Found"
...
-- Build the human-facing URL for a Confluence page from a content object.
-- _links.webui is the canonical UI path (e.g. "/spaces/PROJ/pages/12345/Title").
local function page_browser_url(auth, page)
    local site = auth.site_url or auth.conf_base:gsub("/wiki$", "")
    if page._links and page._links.webui then return site .. "/wiki" .. page._links.webui end
...
local function page_to_row(auth, page)
    ...
    return {
        ...
        actions = {
            ...
            { label = "Copy title", kind = "clipboard", args = { page.title or "" } },
        },
    }
end
...
lark.register({
    on_run = function()
        local auth, err = atlassian_auth("Recent Pages")
        ...
        for _, page in ipairs(data.results or {}) do
            items[#items + 1] = page_to_row(auth, page)
        end
        if #items == 0 then
            items[#items + 1] = { label = "No pages found", icon = "📭" }
        end
        return {
            title = string.format("Recent Pages — %d", #items),
            items = items,
        }
    end,
})
