...
Sourced from: an 8-subsystem architecture map, a 94-agent bug bash (59 confirmed findings, 22/22 architecture claims confirmed, per-finding adversarial verification), and two GPT-5.6-Sol (max) adversarial passes. Fix order: (W1) instrument + amplification fixes + crash containment; (W2) execution identity + cache/streaming hardening; (W3) UI-task isolation; (W4) persistence + state invariants; (W5) agent minimum hardening; (W6) launch-surface coherence + rehearsal. Launch sequencing gate: execution IDs → async dispatch; the full stable-ID migration is deferred to v1.1.
